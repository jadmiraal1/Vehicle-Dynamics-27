...
```matlab
build_tire_coeffs;             % ONLY when TTC data or the fit changes
vd_selftest                    % regression check - run after ANY edit
ttc_fit;                       % tire SCREENING (camber/pressure windows)
pacejka_fit;                   % Magic Formula fit - the design-grip source
run_load_transfer_targets;     % CG ceiling, track floor, brake bias
run_gg_targets;                % skidpad, accel, g-g envelope, power
run_lap_targets;               % lap times, energy, full-lap mass sensitivity
run_handling_targets;          % understeer gradient, yaw response
```

```bash
cd tracks && python digitize_track.py --all    # course PNGs -> track CSVs
```

All scripts are standalone (each calls `vehicle_params()` itself) and write
their plots as PNGs into `plots/`.
